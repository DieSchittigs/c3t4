...
function bundleName (options) {
  // This method mimicks the parseClassName function of Symfony bundles
  // https://github.com/symfony/http-kernel/blob/master/Bundle/Bundle.php#L171
  return options.bundleName.toLowerCase()
}

async function adjustModule (modulePath, options) {
  const pResolve = resolve

  const moduleAssetsPath = `system/modules/${options.name}/assets/`
  const bundleAssetsPath = `bundles/${bundleName(options)}/`

  console.log(chalk.cyan(`Need to replace ${chalk.dim(moduleAssetsPath)} with ${chalk.dim(bundleAssetsPath)} in all module files.`))
...
(async function () {
  ...
  await Promise.reduce(modules, async (irrelevant, {name, path}) => {
    ...
    try {
      options.name = name
      const contaoPath = resolve(resolvedBundlePath, 'src/Resources/contao')
      await copyFixtures(fixturePath, resolvedBundlePath, replacer)
      await copyModule(path, contaoPath)
      await adjustModule(contaoPath, options)
    } catch (err) {
    ...
    added.push({
      name: options.composerName,
      dir: './' + relative(process.cwd(), resolve(basicOptions.newBundlePath, options.targetDir))
    })
  ...
  if (failed.length) {
    ...
    console.log(chalk.bold(`\nHere is an example for how to integrate your bundles into your ${chalk.underline('composer.json')}.`))
    const requirements = added.map(i => `        "${i.name}": "@dev"`).join(',\n')
    const paths = added.map(i => `        {"type": "path", "url": "${i.dir}"}`).join(',\n')
